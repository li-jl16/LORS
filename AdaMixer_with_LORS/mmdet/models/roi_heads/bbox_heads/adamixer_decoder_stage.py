...
class AdaptiveSampling(nn.Module):
    ...
    def forward(self, x, query_xyzr, query_feat, featmap_strides):
        ...
        if DEBUG:
            torch.save(
                sample_points_xyz, 'demo/sample_xy_{}.pth'.format(AdaptiveSampling._DEBUG))
        ...
        if DEBUG:
            torch.save(
                sampled_feature, 'demo/sample_feature_{}.pth'.format(AdaptiveSampling._DEBUG))
            AdaptiveSampling._DEBUG += 1
...
class AdaMixerDecoderStage(BBoxHead):
    ...
    def __init__(self,
        ...
        self.sampling = AdaptiveSampling(
    ...
    @auto_fp16()
    def feat_sampling(self,
                x,
                query_xyzr,
                query_content,
                featmap_strides):
        ''' adaptive 3D sampling '''
        feature_sampled = self.sampling(x, 
                                        query_xyzr, 
                                        query_content, 
                                        featmap_strides)
    ...
    @auto_fp16()
    def forward(self, query_content):
        N, n_query = query_content.shape[:2]
